...
# the new approach proposed in the paper. associative embedding grouping
import numpy as np
from munkres import Munkres
import torch

def py_max_match(scores):
    # it is Munkres assignment algorithm
    # generate index matrix to solve mininum problem
    m = Munkres()
...
def match_by_tag(inp, params, pad=False):
    # used in class HeatmapParser().match()
    tag_k, loc_k, val_k = inp
    assert type(params) is Params
    default_ = np.zeros((params.num_parts, 3 + tag_k.shape[2]))
    # dic is containing tags(keys) and values, dic2 is only containing tags(keys)
    dic = {}
    dic2 = {}
    # iterate over the number of body parts
    for i in range(params.num_parts):
        # grouping tags within one human pose estimation
        # for current body part index and tag
        ptIdx = params.partOrder[i]
        tags = tag_k[ptIdx]

        joints = np.concatenate((loc_k[ptIdx], val_k[ptIdx, :, None], tags), 1)
        # larger than params.detection_threshold which is 1 then do mask(True)
        mask = joints[:, 2] > params.detection_threshold
        tags = tags[mask]
        joints = joints[mask]
        if i == 0 or len(dic) == 0:
            # if it is first run then initialize dic, dic2
            ...
            # params.ignore_too_much be set True in class HeatmapParser().
            # either larger than 30 people or difference of joints and tags dimension is zero will jumpy this loop
            ...
            for row, col in pairs:
                # for each dic, dic2 add 'keys' and 'values'
    ...
    if pad:
        # padding extra zeros to get max_num_people annotation array
        num = len(ans)
...
class HeatmapParser():
    ...
    def nms(self, det):
        ...
        maxm = self.pool(det)
        # torch.eq compute element-wise equality
    ...
    def calc(self, det, tag):
        # set volatile True is to not tracked by autograd, but in new version of pytorch, it has no effect
        ...
        # val (1, 17, 128*128)
        # tag (1, 17, 128*128, -1)
        # return the top 30(max_num_people)
        val_k, ind = det.topk(self.param.max_num_people, dim=2)
        # gather tag value along dim=3 and concatenate in sequence
    ...
    def adjust(self, ans, det):
        # go over the annotations to adjust the points' position
    ...
    def parse(self, det, tag, adjust=True):
        # main function in this class, all function will be called while execute this
