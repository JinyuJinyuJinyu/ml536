...
int my_lib_loss_forward(THCudaTensor *Tag, THLongTensor *keypoints, THFloatTensor *output, THFloatTensor *mean_tags){
    //Tag: batch * (feature*w*h) x tag_dim
    //keypoints: batch * people * 17 * 2
    //output: batch * 2
    ...
    // iterate over batchsize of images and within iteration to iterate over the number of people in batchsize images
    // and also a iteration for each people's joint parts
...
int my_lib_loss_backward(THCudaTensor *Tag, THLongTensor *keypoints, THFloatTensor *mean_tags, THFloatTensor *grad_output, THCudaTensor *grad_input){
    ...
    // THFloatTensor_data returns a pointer to the tensor where it stored
    real* batch_mean = THFloatTensor_data(mean_tags);
