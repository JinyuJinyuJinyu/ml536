...
class HeatmapLoss(torch.nn.Module):
    # from the paper 'Human pose estimation via Convolutional Part Heatmap Regression', it imply that
    # by using the way of heatmap to guide the network concentrate in important part of image
    # in this paper, I think it somehow inherit that one, by improving grouping technique improve accuracy
    """
    loss for detection heatmap
    mask is used to mask off the crowds in coco dataset
    """
    # used in posenet.py to calculate detection loss
    def __init__(self):
