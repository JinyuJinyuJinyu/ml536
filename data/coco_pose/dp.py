...
class GenerateHeatmap():
    ...
    def __call__(self, keypoints):
        # empty 3d matrix to store heatmap
        hms = np.zeros(shape=(self.num_parts, self.output_res, self.output_res), dtype = np.float32)
        sigma = self.sigma
        # go through each pixels, get heat value
        for p in keypoints:
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config, ds, index):
        # config setup from task /pose.py
    ...
    def loadImage(self, idx):
        ...
        # get mask, annotations and keypoints by using function from ref.py
...
def init(config):
    ...
    def gen(phase):
        # return a dictionary of images, mask, heatmap, keypoints
        batchsize = config['train']['batchsize']
