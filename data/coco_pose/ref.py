...
# coco database directory for images and annotations, and this py file's path
data_dir = '/media/jinyu/aae9e04d-ddfd-4048-ba75-be794af6c0b8/jinyu/train2017/'
ann_path = '/home/jinyu/Downloads/annotations/person_keypoints_train2017.json'
ref_dir = os.path.dirname(__file__)
# check the file path exist or not,for loading data, if not raise error
assert os.path.exists(data_dir)
assert os.path.exists(ann_path)
coco, img_ids, num_examples = None, None, None
# load valid id for validation after each epoch
with open(ref_dir + '/valid_id', 'r') as f:
    valid_id = list(map(lambda x:int(x.strip()), f.readlines()))
valid_id_set = set(valid_id)

def init():
    # using pycocotools.coco to load annotations
...
# code for define the part of human body to estimate
num_parts = 17
# the mask, 0 will not show on image, 1 will show
...
# 17 classifications for CNN output
part_idx = {b:a for a, b in enumerate(part_labels)}
# the order of the parts_lables, turned into index number
...
def image_path(idx):
    # function to return current image path
    # used in function 'load_image()'
...
def num_objects(idx, anns=None, should_greater_than_1 = False):
    # function to return number of object in annotations
    # used in function 'get_keypoints()'
...
def get_mask(idx):
    # get current images annotations,ids, using pycocotools to decode rel file,
    ann_ids = coco.getAnnIds(imgIds=img_ids[idx])
...
def get_keypoints(idx, anns=None):
    # get current image's annotation
    if anns is None: anns = get_anns(idx)
    num_people = num_objects(idx, anns)
    # declare matrix for storing 17 classifications or keypoints info
    kps = np.zeros((num_people, 17, 3))
    # go through whole annotations and assign keypoints to kp[]
